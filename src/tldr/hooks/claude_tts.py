...
def main() -> int:
    ...
    payload_log = Path(
        os.environ.get("CLAUDE_TTS_PAYLOAD_LOG", "/tmp/claude-tts-payload.json")
    )
    state_file = Path(
        os.environ.get(
            "CLAUDE_TTS_STATE_FILE", str(Path.home() / ".claude/tts.enabled")
        )
    )
    ...
    session_id = os.environ.get("CLAUDE_TTS_SESSION_ID", "") or first_string(
