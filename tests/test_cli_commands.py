...
class CliCommandTests(unittest.TestCase):
    ...
    def _run_cli_with_home(self, argv, home):
        ...
        stderr = io.StringIO()
        with (
            mock.patch("tldr.cli.Path.home", return_value=home),
    ...
    def test_init_config_writes_remote_config_to_user_home(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path = home / ".config" / "tldr" / "config.toml"
            self.assertEqual(code, 0)
    ...
    def test_init_config_writes_local_profile(self):
        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp) / "home"
            home.mkdir()

            code, stdout, stderr = self._run_cli_with_home(
                ["init-config", "--profile", "local"], home
            )
